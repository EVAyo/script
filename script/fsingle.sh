...
fclone lsf "$fclone_name2":{$link1} --format "pi" --files-only -R > /root/fclone_shell_bot/log/fsingle_task.txt
IFS=$'\n'
suma=0
...
fclone copy "$fclone_name2":{$input_id} "$fclone_name2":{$link2} --drive-server-side-across-configs --fast-list --no-traverse --size-only --stats=1s --stats-one-line -P --drive-pacer-min-sleep=1ms --ignore-checksum --ignore-existing --buffer-size=50M --use-mmap --log-level=ERROR --log-file=/root/fclone_shell_bot/log/fsingle.log
done
