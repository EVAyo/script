...
: > /root/fclone_shell_bot/log/fqcopy1.log
: > /root/fclone_shell_bot/log/fqcopy2.log

clear
read -p "【极速转存模式】，请输入分享链接==>" link
link=${link#*id=};link=${link#*folders/};link=${link#*d/};link=${link%?usp*}
rootname=$(fclone lsd "$fclone_name1":{$link} --dump bodies -vv 2>&1 | awk 'BEGIN{FS="\""}/^{"id/{print $8}')
if [ -z "$link" ] ; then
echo "不允许输入为空" && exit ;
elif [ -z "$rootname" ] ; then
echo -e "读取文件夹名称出错，请反馈问题给作者,如果是全盘请用fb,此模式读不了盘名/n" && exit ;
else
echo -e "▣▣▣▣▣▣▣任务信息▣▣▣▣▣▣▣\n"
