...
[task_local]
#京喜牧场
20 * * * * jd_jxmc.js, tag=京喜牧场, img-url=https://github.com/58xinian/icon/raw/master/jdgc.png, enabled=true

================Loon==============
[Script]
cron "20 * * * *" script-path=jd_jxmc.js,tag=京喜牧场

===============Surge=================
京喜牧场 = type=cron,cronexp="20 * * * *",wake-system=1,timeout=3600,script-path=jd_jxmc.js

============小火箭=========
京喜牧场 = type=cron,script-path=jd_jxmc.js, cronexpr="20 * * * *", timeout=3600, enable=true
 */
...
!(async () => {
  ...
  $.res = await getAuthorShareCode('https://action-1251995682.file.myqcloud.com/shareCodes/jxmc.json')
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://gitee.com/dockere/jd-base/raw/master/null.json`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
...
function uploadShareCode(code) {
  return new Promise(async resolve => {
    $.post({url: `https://gitee.com/dockere/jd-base/raw/master/null.json/jxmc?code=${code}&ptpin=${encodeURIComponent(encodeURIComponent($.UserName))}`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
