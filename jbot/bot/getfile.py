...
async def my_file(event):
    ...
    try:
        ...
        if event.message.file:
            markup = []
            filename = f'bot_{event.message.file.name}'
            cmdtext = None
            async with jdbot.conversation(SENDER, timeout=30) as conv:
                ...
                if res == 'cancel':
                    ...
                    if res == 'node':
                        ...
                        await jdbot.download_media(event.message, f'{_DiyDir}/{filename}')
                        cmdtext = f'{jdcmd} {filename} now'
                        ...
                        await jdbot.download_media(event.message, f'{_ScriptsDir}/{filename}')
                        ...
                        await jdbot.download_media(event.message, f'{res}/{filename}')
