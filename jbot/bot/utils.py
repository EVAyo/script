...
def upcron(cron):
    owninfo = '# 运行lxk0301大佬的js脚本，仅列出长期任务作初始化用，AutoAddCron=true时，将自动添加短期任务。'
...
async def cronup(jdbot, conv, resp, filename, msg, SENDER, markup, path):
    try:
        if QL:
            ...
            crondata = f'{mycron(resp)} bash jd {filename}'
        ...
        if QL:
            ...
            crondata = f'0 0 * * * bash jd {filename}'
        ...
        if fun == 'search':
            ...
            for cron in v4crons:
                if str(crondata) in cron:
                    res['data'][cron.split(
                        'bash jd ')[-1].split(' ')[0].split('/')[-1].replace('\n', '')] = cron
        elif fun == 'add':
