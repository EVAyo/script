...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN('https://action-1251995682.file.myqcloud.com/shareCodes/jd_updateFactoryTuanId.json')
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://gitee.com/dockere/jd-base/raw/master/null.json`, headers: {'Host': 'api.jdsharecode.xyz'}, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
